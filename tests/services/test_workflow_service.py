...
class TestWorkflowService:
    ...
    async def test_create(self, service, mock_repository):
        ...
        obj_id = uuid.uuid4()
        mock_obj = MagicMock(spec=Workflow)
        mock_obj.id = obj_id
    ...
    async def test_authorization_update_denied(self, service, mock_repository):
        ...
        with patch.object(service, "_authorize_update", return_value=False):
            with pytest.raises(PermissionError):
                await service.update(uuid.uuid4(), WorkflowUpdate())
    ...
    async def test_cache_invalidates_on_update(self, service, mock_repository):
        ...
        mock_repository.get.return_value = mock_obj
        with patch.object(service, "_cache_invalidate") as mock_inv:
            await service.update(obj_id, WorkflowUpdate())
    ...
    async def test_event_published_on_create(self, service, mock_repository):
        ...
        EventBus.register("Workflow.Created", collector)
        await service.create(WorkflowCreate())
    ...
    def test_metadata_constants_exist(self):
        ...
        assert hasattr(WorkflowService, "CACHE_ENABLED")
        assert hasattr(WorkflowService, "CACHE_TTL")
        assert hasattr(WorkflowService, "PERMISSIONS")
        assert hasattr(WorkflowService, "FEATURE_FLAGS")
        assert hasattr(WorkflowService, "EVENTS")
        assert hasattr(WorkflowService, "RATE_LIMIT")
        assert hasattr(WorkflowService, "DEPENDENCIES")
