...
class TestProjectService:
    ...
    async def test_create(self, service, mock_repository):
        ...
        data = ProjectCreate(organization_id=str(uuid.uuid4()), name="Test")
    ...
    async def test_authorization_create_denied(self, service, mock_repository):
        ...
        with patch.object(service, "_authorize_create", return_value=False):
            with pytest.raises(PermissionError):
                await service.create(ProjectCreate(organization_id=str(uuid.uuid4()), name="Test"))
    ...
    async def test_cache_invalidates_on_update(self, service, mock_repository):
        ...
        mock_repository.get.return_value = mock_obj
        with patch.object(service, "_cache_invalidate") as mock_inv:
            await service.update(obj_id, ProjectUpdate())
    ...
    async def test_event_published_on_create(self, service, mock_repository):
        ...
        subscribe("Project.Created", collector)
        try:
            await service.create(ProjectCreate(organization_id=str(uuid.uuid4()), name="Test"))
            assert len(events) > 0, "No events were published"
        finally:
            unsubscribe("Project.Created", collector)
    ...
    def test_metadata_constants_exist(self):
        ...
        assert hasattr(ProjectService, "CACHE_ENABLED")
        assert hasattr(ProjectService, "CACHE_TTL")
        assert hasattr(ProjectService, "PERMISSIONS")
        assert hasattr(ProjectService, "FEATURE_FLAGS")
        assert hasattr(ProjectService, "EVENTS")
        assert hasattr(ProjectService, "RATE_LIMIT")
        assert hasattr(ProjectService, "DEPENDENCIES")
