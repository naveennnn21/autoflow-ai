"""Tests for the WorkflowNodeService."""
...
class TestWorkflowNodeService:
    ...
    async def test_exists(self, service, mock_repository):
        """Test checking if workflownode exists."""
    ...
    async def test_authorization_create_denied(self, service, mock_repository):
        ...
        with patch.object(service, "_authorize_create", return_value=False):
            with pytest.raises(PermissionError):
                await service.create(WorkflowNodeCreate(workflow_id=str(uuid.uuid4()), type='trigger', label="Test"))
    ...
    async def test_authorization_update_denied(self, service, mock_repository):
        ...
        with patch.object(service, "_authorize_update", return_value=False):
            with pytest.raises(PermissionError):
                await service.update(uuid.uuid4(), WorkflowNodeUpdate())
    ...
    async def test_cache_invalidates_on_update(self, service, mock_repository):
        ...
        mock_repository.get.return_value = mock_obj
        with patch.object(service, "_cache_invalidate") as mock_inv:
            await service.update(obj_id, WorkflowNodeUpdate())
    ...
    def test_metadata_constants_exist(self):
        ...
        assert hasattr(WorkflowNodeService, "CACHE_ENABLED")
        assert hasattr(WorkflowNodeService, "CACHE_TTL")
        assert hasattr(WorkflowNodeService, "PERMISSIONS")
        assert hasattr(WorkflowNodeService, "FEATURE_FLAGS")
        assert hasattr(WorkflowNodeService, "EVENTS")
        assert hasattr(WorkflowNodeService, "RATE_LIMIT")
        assert hasattr(WorkflowNodeService, "DEPENDENCIES")
