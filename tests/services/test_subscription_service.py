...
def service(mock_repository):
    ...
    return SubscriptionService(mock_repository)
...
class TestSubscriptionService:
    ...
    async def test_create(self, service, mock_repository):
        ...
        obj_id = uuid.uuid4()
        mock_obj = MagicMock(spec=Subscription)
        mock_obj.id = obj_id
        mock_repository.create.return_value = mock_obj
        mock_repository.get.return_value = None
        data = SubscriptionCreate(organization_id=str(uuid.uuid4()), plan_id="Test", current_period_start="2026-01-01T00:00:00", current_period_end="2026-01-01T00:00:00")
    ...
    async def test_bulk_create(self, service, mock_repository):
        """Test bulk creating subscriptions."""
    ...
    async def test_authorization_create_denied(self, service, mock_repository):
        ...
        with patch.object(service, "_authorize_create", return_value=False):
            with pytest.raises(PermissionError):
                await service.create(SubscriptionCreate(organization_id=str(uuid.uuid4()), plan_id="Test", current_period_start="2026-01-01T00:00:00", current_period_end="2026-01-01T00:00:00"))
    ...
    async def test_authorization_update_denied(self, service, mock_repository):
        ...
        with patch.object(service, "_authorize_update", return_value=False):
            with pytest.raises(PermissionError):
                await service.update(uuid.uuid4(), SubscriptionUpdate())
    ...
    async def test_cache_invalidates_on_update(self, service, mock_repository):
        ...
        mock_repository.get.return_value = mock_obj
        with patch.object(service, "_cache_invalidate") as mock_inv:
            await service.update(obj_id, SubscriptionUpdate())
    ...
    async def test_event_published_on_create(self, service, mock_repository):
        ...
        subscribe("Subscription.Created", collector)
        try:
            await service.create(SubscriptionCreate(organization_id=str(uuid.uuid4()), plan_id="Test", current_period_start="2026-01-01T00:00:00", current_period_end="2026-01-01T00:00:00"))
            assert len(events) > 0, "No events were published"
        finally:
            unsubscribe("Subscription.Created", collector)
    ...
    def test_metadata_constants_exist(self):
        ...
        assert hasattr(SubscriptionService, "CACHE_ENABLED")
        assert hasattr(SubscriptionService, "CACHE_TTL")
        assert hasattr(SubscriptionService, "PERMISSIONS")
        assert hasattr(SubscriptionService, "FEATURE_FLAGS")
        assert hasattr(SubscriptionService, "EVENTS")
        assert hasattr(SubscriptionService, "RATE_LIMIT")
        assert hasattr(SubscriptionService, "DEPENDENCIES")
    ...
    async def test_restore(self, service, mock_repository):
        """Test restoring a soft-deleted subscription."""
        obj_id = uuid.uuid4()
        mock_obj = MagicMock(spec=Subscription)
        mock_obj.id = obj_id
