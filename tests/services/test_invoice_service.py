...
def service(mock_repository):
    ...
    return InvoiceService(mock_repository)


class TestInvoiceService:
    """Test suite for InvoiceService."""
    ...
    async def test_authorization_create_denied(self, service, mock_repository):
        ...
        with patch.object(service, "_authorize_create", return_value=False):
            with pytest.raises(PermissionError):
                await service.create(InvoiceCreate(organization_id=str(uuid.uuid4()), amount=100.0))
    ...
    async def test_authorization_update_denied(self, service, mock_repository):
        ...
        with patch.object(service, "_authorize_update", return_value=False):
            with pytest.raises(PermissionError):
                await service.update(uuid.uuid4(), InvoiceUpdate())
    ...
    async def test_cache_invalidates_on_update(self, service, mock_repository):
        ...
        mock_repository.get.return_value = mock_obj
        with patch.object(service, "_cache_invalidate") as mock_inv:
            await service.update(obj_id, InvoiceUpdate())
    ...
    async def test_event_published_on_create(self, service, mock_repository):
        ...
        subscribe("Invoice.Created", collector)
        try:
            await service.create(InvoiceCreate(organization_id=str(uuid.uuid4()), amount=100.0))
            assert len(events) > 0, "No events were published"
        finally:
            unsubscribe("Invoice.Created", collector)
    ...
    def test_metadata_constants_exist(self):
        ...
        assert hasattr(InvoiceService, "CACHE_ENABLED")
        assert hasattr(InvoiceService, "CACHE_TTL")
        assert hasattr(InvoiceService, "PERMISSIONS")
        assert hasattr(InvoiceService, "FEATURE_FLAGS")
        assert hasattr(InvoiceService, "EVENTS")
        assert hasattr(InvoiceService, "RATE_LIMIT")
        assert hasattr(InvoiceService, "DEPENDENCIES")
