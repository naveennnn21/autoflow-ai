...
class TestTemplateService:
    ...
    async def test_create(self, service, mock_repository):
        ...
        obj_id = uuid.uuid4()
        mock_obj = MagicMock(spec=Template)
        mock_obj.id = obj_id
        mock_repository.create.return_value = mock_obj
        mock_repository.get.return_value = None
        data = TemplateCreate(organization_id=str(uuid.uuid4()), name="Test", slug="Test")
    ...
    async def test_authorization_create_denied(self, service, mock_repository):
        ...
        with patch.object(service, "_authorize_create", return_value=False):
            with pytest.raises(PermissionError):
                await service.create(TemplateCreate(organization_id=str(uuid.uuid4()), name="Test", slug="Test"))
    ...
    async def test_authorization_update_denied(self, service, mock_repository):
        ...
        with patch.object(service, "_authorize_update", return_value=False):
            with pytest.raises(PermissionError):
                await service.update(uuid.uuid4(), TemplateUpdate())
    ...
    async def test_cache_invalidates_on_update(self, service, mock_repository):
        ...
        mock_repository.get.return_value = mock_obj
        with patch.object(service, "_cache_invalidate") as mock_inv:
            await service.update(obj_id, TemplateUpdate())
    ...
    async def test_event_published_on_create(self, service, mock_repository):
        ...
        subscribe("Template.Created", collector)
        try:
            await service.create(TemplateCreate(organization_id=str(uuid.uuid4()), name="Test", slug="Test"))
            assert len(events) > 0, "No events were published"
        finally:
            unsubscribe("Template.Created", collector)
    ...
    def test_metadata_constants_exist(self):
        ...
        assert hasattr(TemplateService, "CACHE_ENABLED")
        assert hasattr(TemplateService, "CACHE_TTL")
        assert hasattr(TemplateService, "PERMISSIONS")
        assert hasattr(TemplateService, "FEATURE_FLAGS")
        assert hasattr(TemplateService, "EVENTS")
        assert hasattr(TemplateService, "RATE_LIMIT")
        assert hasattr(TemplateService, "DEPENDENCIES")
