"""Tests for the MarketplaceItemService."""
...
from app.schemas.marketplace_item import MarketplaceItemCreate, MarketplaceItemUpdate


@pytest_asyncio.fixture
def mock_repository():
    """Create a mock repository for testing."""
    repo = AsyncMock(spec=MarketplaceItemRepository)
    repo.transaction.return_value.__aenter__.return_value = None
    repo.transaction.return_value.__aexit__.return_value = None
    repo.model_class = MarketplaceItem
    return repo
...
def service(mock_repository):
    ...
    return MarketplaceItemService(mock_repository)


class TestMarketplaceItemService:
    """Test suite for MarketplaceItemService."""

    # --- Core CRUD tests ---

    async def test_create(self, service, mock_repository):
        """Test creating a new marketplaceitem."""
        obj_id = uuid.uuid4()
        mock_obj = MagicMock(spec=MarketplaceItem)
        mock_obj.id = obj_id
        mock_repository.create.return_value = mock_obj
        mock_repository.get.return_value = None
        data = MarketplaceItemCreate(name="Test", slug="Test", category="Test")
        result = await service.create(data)
        assert result is not None
        assert result.id == obj_id

    async def test_get(self, service, mock_repository):
        """Test retrieving a marketplaceitem."""
        obj_id = uuid.uuid4()
        mock_obj = MagicMock(spec=MarketplaceItem)
        mock_obj.id = obj_id
    ...
    async def test_delete(self, service, mock_repository):
        """Test deleting a marketplaceitem."""
        obj_id = uuid.uuid4()
        mock_obj = MagicMock(spec=MarketplaceItem)
        mock_obj.id = obj_id
        mock_repository.get.return_value = mock_obj
    ...
    async def test_exists(self, service, mock_repository):
        """Test checking if marketplaceitem exists."""
    ...
    async def test_authorization_create_denied(self, service, mock_repository):
        ...
        with patch.object(service, "_authorize_create", return_value=False):
            with pytest.raises(PermissionError):
                await service.create(MarketplaceItemCreate(name="Test", slug="Test", category="Test"))
    ...
    async def test_authorization_update_denied(self, service, mock_repository):
        ...
        with patch.object(service, "_authorize_update", return_value=False):
            with pytest.raises(PermissionError):
                await service.update(uuid.uuid4(), MarketplaceItemUpdate())
    ...
    async def test_cache_invalidates_on_update(self, service, mock_repository):
        ...
        mock_repository.get.return_value = mock_obj
        with patch.object(service, "_cache_invalidate") as mock_inv:
            await service.update(obj_id, MarketplaceItemUpdate())
    ...
    async def test_event_published_on_create(self, service, mock_repository):
        ...
        subscribe("MarketplaceItem.Created", collector)
        try:
            await service.create(MarketplaceItemCreate(name="Test", slug="Test", category="Test"))
            assert len(events) > 0, "No events were published"
        finally:
            unsubscribe("MarketplaceItem.Created", collector)
    ...
    def test_metadata_constants_exist(self):
        ...
        assert hasattr(MarketplaceItemService, "CACHE_ENABLED")
        assert hasattr(MarketplaceItemService, "CACHE_TTL")
        assert hasattr(MarketplaceItemService, "PERMISSIONS")
        assert hasattr(MarketplaceItemService, "FEATURE_FLAGS")
        assert hasattr(MarketplaceItemService, "EVENTS")
        assert hasattr(MarketplaceItemService, "RATE_LIMIT")
        assert hasattr(MarketplaceItemService, "DEPENDENCIES")

    async def test_restore(self, service, mock_repository):
        """Test restoring a soft-deleted marketplaceitem."""
        obj_id = uuid.uuid4()
        mock_obj = MagicMock(spec=MarketplaceItem)
        mock_obj.id = obj_id
