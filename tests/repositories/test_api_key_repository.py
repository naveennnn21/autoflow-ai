...
class TestAPIKeyRepository:
    ...
    async def test_get(self, repo, db_session):
        """Test retrieving a apikey by ID."""
        obj_id = uuid.uuid4()
    ...
    async def test_get_not_found(self, repo, db_session):
        """Test retrieving a non-existent apikey."""
        mock_result = MagicMock()
        mock_result.scalar_one_or_none.return_value = None
        db_session.execute.return_value = mock_result
    ...
    async def test_exists(self, repo, db_session):
        """Test checking if a apikey exists."""
        mock_result = MagicMock()
    ...
    async def test_count(self, repo, db_session):
        """Test counting apikeys with tenant filter."""
        mock_result = MagicMock()
    ...
    async def test_search_pagination(self, repo, db_session):
        """Test searching and paginating apikeys."""
        mock_obj = MagicMock(spec=APIKey)
        mock_result = MagicMock()
    ...
    async def test_get_by_key_hash(self, repo, db_session):
        """Test looking up a apikey by key_hash."""
        mock_obj = MagicMock(spec=APIKey)
        mock_result = MagicMock()
        mock_result.scalar_one_or_none.return_value = mock_obj
        db_session.execute.return_value = mock_result

        result = await repo.get_by_key_hash("test_value")
        assert result is not None
