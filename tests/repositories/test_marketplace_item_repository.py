"""Tests for the MarketplaceItemRepository."""
...
def repo(db_session):
    ...
    return MarketplaceItemRepository(db_session)


class TestMarketplaceItemRepository:
    """Test suite for MarketplaceItemRepository."""
    ...
    async def test_get(self, repo, db_session):
        """Test retrieving a marketplaceitem by ID."""
        obj_id = uuid.uuid4()
    ...
    async def test_get_not_found(self, repo, db_session):
        """Test retrieving a non-existent marketplaceitem."""
        mock_result = MagicMock()
        mock_result.scalar_one_or_none.return_value = None
        db_session.execute.return_value = mock_result
    ...
    async def test_delete_soft(self, repo, db_session):
        """Test soft deleting a marketplaceitem."""
        obj_id = uuid.uuid4()
        mock_obj = MagicMock(spec=MarketplaceItem)
        mock_obj.id = obj_id
        mock_result = MagicMock()
    ...
    async def test_delete_hard(self, repo, db_session):
        """Test hard deleting a marketplaceitem."""
        obj_id = uuid.uuid4()
        mock_obj = MagicMock(spec=MarketplaceItem)
        mock_obj.id = obj_id
        mock_result = MagicMock()
    ...
    async def test_exists(self, repo, db_session):
        """Test checking if a marketplaceitem exists."""
        mock_result = MagicMock()
    ...
    async def test_count(self, repo, db_session):
        """Test counting marketplaceitems with tenant filter."""
        mock_result = MagicMock()
    ...
    async def test_search_pagination(self, repo, db_session):
        """Test searching and paginating marketplaceitems."""
        mock_obj = MagicMock(spec=MarketplaceItem)
        mock_result = MagicMock()
    ...
    async def test_bulk_delete(self, repo, db_session):
        """Test bulk deleting marketplaceitems."""
    ...
    async def test_get_by_slug(self, repo, db_session):
        """Test looking up a marketplaceitem by slug."""
        mock_obj = MagicMock(spec=MarketplaceItem)
        mock_result = MagicMock()
        mock_result.scalar_one_or_none.return_value = mock_obj
        db_session.execute.return_value = mock_result

        result = await repo.get_by_slug("test_value")
        assert result is not None
