"""Tests for the OrganizationRepository."""
...
class TestOrganizationRepository:
    ...
    async def test_get(self, repo, db_session):
        """Test retrieving a organization by ID."""
        obj_id = uuid.uuid4()
    ...
    async def test_delete_soft(self, repo, db_session):
        """Test soft deleting a organization."""
        obj_id = uuid.uuid4()
        mock_obj = MagicMock(spec=Organization)
        mock_obj.id = obj_id
        mock_result = MagicMock()
    ...
    async def test_delete_hard(self, repo, db_session):
        """Test hard deleting a organization."""
        obj_id = uuid.uuid4()
        mock_obj = MagicMock(spec=Organization)
        mock_obj.id = obj_id
        mock_result = MagicMock()
    ...
    async def test_exists(self, repo, db_session):
        """Test checking if a organization exists."""
        mock_result = MagicMock()
    ...
    async def test_count(self, repo, db_session):
        """Test counting organizations with tenant filter."""
        mock_result = MagicMock()
    ...
    async def test_search_pagination(self, repo, db_session):
        """Test searching and paginating organizations."""
        mock_obj = MagicMock(spec=Organization)
        mock_result = MagicMock()
    ...
    async def test_get_by_slug(self, repo, db_session):
        """Test looking up a organization by slug."""
        mock_obj = MagicMock(spec=Organization)
        mock_result = MagicMock()
        mock_result.scalar_one_or_none.return_value = mock_obj
        db_session.execute.return_value = mock_result
