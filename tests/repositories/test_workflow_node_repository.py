"""Tests for the WorkflowNodeRepository."""
...
def repo(db_session):
    ...
    return WorkflowNodeRepository(db_session)


class TestWorkflowNodeRepository:
    """Test suite for WorkflowNodeRepository."""
    ...
    async def test_get(self, repo, db_session):
        """Test retrieving a workflownode by ID."""
        obj_id = uuid.uuid4()
    ...
    async def test_get_not_found(self, repo, db_session):
        """Test retrieving a non-existent workflownode."""
        mock_result = MagicMock()
        mock_result.scalar_one_or_none.return_value = None
        db_session.execute.return_value = mock_result
    ...
    async def test_delete_soft(self, repo, db_session):
        """Test soft deleting a workflownode."""
        obj_id = uuid.uuid4()
        mock_obj = MagicMock(spec=WorkflowNode)
        mock_obj.id = obj_id
        mock_result = MagicMock()
    ...
    async def test_delete_hard(self, repo, db_session):
        """Test hard deleting a workflownode."""
        obj_id = uuid.uuid4()
        mock_obj = MagicMock(spec=WorkflowNode)
        mock_obj.id = obj_id
        mock_result = MagicMock()
    ...
    async def test_exists(self, repo, db_session):
        """Test checking if a workflownode exists."""
        mock_result = MagicMock()
    ...
    async def test_count(self, repo, db_session):
        """Test counting workflownodes with tenant filter."""
        mock_result = MagicMock()
    ...
    async def test_search_pagination(self, repo, db_session):
        """Test searching and paginating workflownodes."""
        mock_obj = MagicMock(spec=WorkflowNode)
        mock_result = MagicMock()
    ...
    async def test_bulk_delete(self, repo, db_session):
        """Test bulk deleting workflownodes."""
