...
class TestSubscriptionAPI:
    ...
    async def test_list_entities(self, auth_headers):
        ...
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
            resp = await client.get("/api/v1/subscription", headers=auth_headers)
    ...
    async def test_get_entity(self, auth_headers):
        ...
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
            resp = await client.get(f"/api/v1/subscription/{uuid.uuid4()}", headers=auth_headers)
    ...
    async def test_create_entity(self, auth_headers):
        ...
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
            resp = await client.post("/api/v1/subscription", json={}, headers=auth_headers)
    ...
    async def test_delete_entity(self, auth_headers):
        ...
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
            resp = await client.delete(f"/api/v1/subscription/{uuid.uuid4()}", headers=auth_headers)
    ...
    async def test_search_entities(self, auth_headers):
        ...
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
            resp = await client.get("/api/v1/subscription/search?q=test", headers=auth_headers)
            assert resp.status_code in (200, 401, 403)
    ...
    async def test_count_entities(self, auth_headers):
        ...
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
            resp = await client.get("/api/v1/subscription/count", headers=auth_headers)
            assert resp.status_code in (200, 401, 403)

    @pytest.mark.asyncio
    async def test_tenant_isolation(self, auth_headers):
        """Test tenant isolation for subscription."""
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
            resp = await client.get("/api/v1/subscription", headers=auth_headers)
    ...
    async def test_restore_entity(self, auth_headers):
        ...
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
            resp = await client.post(f"/api/v1/subscription/{uuid.uuid4()}/restore", headers=auth_headers)
            assert resp.status_code in (200, 401, 403, 404)
