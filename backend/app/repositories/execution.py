...
Generated repository for the Execution entity.
Consumes metadata/repositories/execution.yaml for configuration.
"""
...
class ExecutionRepository(BaseRepository[Execution]):
    """Repository for Execution entity.

    Implements IRepository[Execution] with full CRUD, search,
    ...
    SEARCH_FIELDS = []
    FILTERABLE_FIELDS = ['status', 'trigger_type', 'retry_attempt']
    SORTABLE_FIELDS = ['started_at', 'completed_at', 'duration_ms', 'cost', 'created_at']
    CACHE_POLICY = "low"
    CACHE_TTL = 120
