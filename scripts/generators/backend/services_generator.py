...
def _build_base_service_content() -> str:
    ...
    Includes cache support, retry decorator, audit logging, and metadata
    inference. Domain events are published through the generated
    metadata-driven bus in app.events (see _publish_event).
    """
    lines = [
        ...
        '            )',
        '            await publish(event)',
        '        except Exception:',  # noqa: BLE001 - never break CRUD on event failure
